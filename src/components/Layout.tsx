...
import { Home, Send, Download, Clock, Zap, QrCode, MessageCircle } from 'lucide-react';
...
export default function Layout() {
  ...
  return (
    <div className="flex min-h-dvh flex-col safe-top relative z-10 overflow-x-hidden">
      ...
      >
        ...
        <div className="flex items-center gap-2.5">
          ...
          <button
            ...
            className="relative flex items-center gap-1.5 rounded-xl border border-white/8 bg-white/5 px-3 py-2 text-xs font-semibold text-slate-300 transition-all hover:bg-white/10 hover:text-slate-100 hover:border-brand-500/30 active:scale-95 duration-200"
            title="Chat & Clipboard Sync"
          >
            ...
            <AnimatePresence>
              {unreadCount > 0 && (
                <motion.span
                  ...
                  animate={{ scale: 1 }}
                  exit={{ scale: 0 }}
                  className="absolute -top-1.5 -right-1.5 flex h-4.5 w-4.5 items-center justify-center rounded-full bg-red-500 text-[8px] font-bold text-white shadow-lg shadow-red-500/30 animate-badge-bounce"
                >
      ...
      {/* Chat Panel Overlay */}
      <ChatPanel isOpen={chatOpen} onClose={handleChatClose} />
        ...
        <AnimatePresence>
          {unreadCount > 0 && (
            <motion.span
              ...
              animate={{ scale: 1 }}
              exit={{ scale: 0 }}
              className="absolute -top-1 -right-1 flex h-5 w-5 items-center justify-center rounded-full bg-red-500 text-[9px] font-bold text-white shadow-lg"
            >
              {unreadCount > 9 ? '9+' : unreadCount}
            </motion.span>
          )}
