...
export function startDiscovery() {
    try {
        ...
        mdnsInstance.on('query', (query) => {
            if (!query.questions)
                return;
            const isForUs = query.questions.some((q) => q.name === SERVICE_NAME ||
        ...
        mdnsInstance.on('response', (response) => {
            ...
            const txtRecord = response.answers.find((a) => a.name === SERVICE_NAME && a.type === 'TXT');
            let deviceIp = '';
            ...
            if (txtRecord && txtRecord.type === 'TXT' && Array.isArray(txtRecord.data)) {
                for (const entry of txtRecord.data) {
                    ...
                    if (str.startsWith('ip='))
                        deviceIp = str.slice(3);
                    if (str.startsWith('port='))
                        devicePort = parseInt(str.slice(5), 10);
                    if (str.startsWith('name='))
                        name = str.slice(5);
                    if (str.startsWith('platform='))
                        devicePlat = str.slice(9);
                }
            }
