...
export const socketDevicesMap = new Map();
 ...
 */
function getClientPublicIp(socket) {
...
export function setupSocketServer(io) {
    io.on('connection', (socket) => {
        ...
        function resolveTargetSocketId(targetId) {
            if (!targetId)
                return null;
        ...
        socket.on('webrtc:offer', (data) => {
            ...
            if (!fromId || !data.targetId || !data.offer)
                return;
        ...
        socket.on('webrtc:answer', (data) => {
            const fromId = socketDevices.get(socket.id);
            if (!fromId || !data.targetId || !data.answer)
                return;
            const targetSocketId = resolveTargetSocketId(data.targetId);
            if (targetSocketId) {
        ...
        socket.on('webrtc:ice', (data) => {
            const fromId = socketDevices.get(socket.id);
            if (!fromId || !data.targetId || !data.candidate)
                return;
            const targetSocketId = resolveTargetSocketId(data.targetId);
            if (targetSocketId) {
