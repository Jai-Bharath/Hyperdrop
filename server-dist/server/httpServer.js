...
const transferLastSeen = new Map();
...
export async function closeFileHandle(transferId) {
...
async function renameWithRetry(src, dest, retries = 10, delay = 100) {
...
// ─── Setup ───────────────────────────────────────────────────────────
export function setupHttpServer(app, io) {
    // Global middleware
    app.use(cors({ origin: '*' }));
    ...
    app.post('/api/chunk', upload.single('chunk'), async (req, res) => {
        try {
            ...
            const { transferId, chunkIndex, totalChunks, fileName, fileSize } = req.body;
            ...
            if (!transferState.has(transferId)) {
                transferState.set(transferId, new Set());
    ...
    app.post('/api/upload-stream', async (req, res) => {
        ...
        const fileSizeStr = req.headers['x-file-size'];
    ...
    app.get('/api/transfer/:id/status', (_req, res) => {
        try {
