...
export const activeTransfers = new Map<string, { senderSocketId: string; receiverSocketId: string }>()
...
export function setupSocketServer(io: Server): void {
  io.on('connection', (socket: Socket) => {
    ...
    socket.on('transfer:done', (data: { id: string }) => {
      const mapping = activeTransfers.get(data.id)
      if (mapping) {
        // Relay done to the other peer (needed for WebRTC P2P transfers)
        const targetSocketId = socket.id === mapping.senderSocketId
          ? mapping.receiverSocketId
          : mapping.senderSocketId
    ...
    // ── Chat & Clipboard Sync Events ────────────────────────────
    // Deduplicate: track which socket IDs have received a given event ID
    // to prevent double-delivery when a device is in multiple rooms
    const recentlyRelayed = new Map<string, Set<string>>() // eventId → Set<socketId>

    function relayToRoomsDeduped(eventName: string, eventId: string, data: any) {
      const deviceId = socketDevices.get(socket.id)
      if (!deviceId) return

      if (!recentlyRelayed.has(eventId)) {
        recentlyRelayed.set(eventId, new Set())
      }
      const alreadySent = recentlyRelayed.get(eventId)!
      ...
      if (rooms) {
        for (const roomName of rooms) {
          ...
          if (!roomMap) continue
          for (const [otherId] of roomMap.entries()) {
            if (otherId === deviceId) continue
            const otherSocketId = deviceSockets.get(otherId)
            if (otherSocketId && !alreadySent.has(otherSocketId)) {
              alreadySent.add(otherSocketId)
              io.to(otherSocketId).emit(eventName, data)
            }
          }
        }
      }
      ...
      setTimeout(() => recentlyRelayed.delete(eventId), 10000)
    }
    ...
    socket.on('chat:message', (data: {
      ...
      relayToRoomsDeduped('chat:message', data.id, data)
      console.log(`[chat] Message from ${data.senderName}: ${data.text.slice(0, 50)}...`)
    })
    ...
    socket.on('chat:typing', (data: { senderId: string; typing: boolean }) => {
      ...
      if (rooms) {
        for (const roomName of rooms) {
          socket.to(roomName).emit('chat:typing', data)
        }
      }
    })

    socket.on('chat:read', (data: { messageId: string; readerId: string }) => {
      const deviceId = socketDevices.get(socket.id)
      if (!deviceId) return
      ...
      if (rooms) {
        for (const roomName of rooms) {
          socket.to(roomName).emit('chat:read', data)
        }
      }
    })
    ...
    socket.on('clipboard:sync', (data: {
      ...
      relayToRoomsDeduped('clipboard:sync', data.id, data)
      console.log(`[clipboard] Sync from ${data.senderName}: ${data.content.slice(0, 50)}...`)
    })
